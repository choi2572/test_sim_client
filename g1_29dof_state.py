            ...
            if now_ms - _obs_cache["dds_last_ms"] >= _obs_cache["dds_min_interval_ms"]:
                ...
                if g1_robot_dds:
                    ...
                    if imu_data.shape[0] > 0:
                        ee_qpos, ee_state_6d = compute_g1_ee_qpos_and_state_6d(env)
...
def _g1_root_body_index(body_names: list[str]) -> int:
    for c in ("pelvis", "base_link", "root", "floating_base"):
        if c in body_names:
            return body_names.index(c)
    return 0


def _g1_first_body_index(body_names: list[str], candidates: tuple[str, ...]) -> int:
    for c in candidates:
        if c in body_names:
            return body_names.index(c)
    raise ValueError(
        f"G1 EE link not found (tried {candidates}). body_names (first 50)={body_names[:50]!r}"
    )


def rot_mat_to_rpy_for_pose17(R: torch.Tensor) -> torch.Tensor:
    """R: [B,3,3] with columns matching rlds_dataset.batch_pose17_to_pose23 convention. Returns [B,3] roll,pitch,yaw."""
    R = R.reshape(-1, 3, 3)
...
def pose17_to_pose23_torch(p17: torch.Tensor) -> torch.Tensor:
    """Same layout as unifolm-vla/prepare_data/hdf5_to_rlds/rlds_dataset.py::batch_pose17_to_pose23 (single row)."""
    v = p17.reshape(17).to(dtype=torch.float64)
    L_xyz = v[0:3]
    L_rpy = v[3:6]
    R_xyz = v[6:9]
    R_rpy = v[9:12]
    ...
    def rpy_to_two_cols(rpy: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        roll, pitch, yaw = rpy[0], rpy[1], rpy[2]
        cr, sr = torch.cos(roll), torch.sin(roll)
        cp, sp = torch.cos(pitch), torch.sin(pitch)
        cy, sy = torch.cos(yaw), torch.sin(yaw)
        col1 = torch.stack([cy * cp, sy * cp, -sp])
        col2 = torch.stack(
            [cy * sp * sr - sy * cr, sy * sp * sr + cy * cr, cp * sr],
        )
        return col1, col2

    L1, L2 = rpy_to_two_cols(L_rpy)
    R1, R2 = rpy_to_two_cols(R_rpy)
    out = torch.cat([L_xyz, L1, L2, R_xyz, R1, R2, waist5])
    return out.to(dtype=torch.float32)


def _g1_gripper_scalar_from_joints(joint_names: list[str], row: torch.Tensor, *, left: bool) -> torch.Tensor:
    """Mean q over hand joints (dex3 left_hand_*, inspire L_* / R_* / left_hand_*)."""
    if left:
        prefs = ("left_hand", "L_")
    else:
        prefs = ("right_hand", "R_")
    idxs = [i for i, n in enumerate(joint_names) if any(n.startswith(p) for p in prefs)]
    if not idxs:
        return row.new_zeros(())
    return row[idxs].mean()


def compute_g1_ee_qpos_and_state_6d(env: ManagerBasedRLEnv) -> tuple[list[float] | None, list[float] | None]:
    """
    Build ee_qpos (17) and ee_state_6d (23) for UnifoLM-VLA EE_R6_G1 proprio.

    - EEF poses: left/right wrist yaw link (or L_ee/R_ee) relative to pelvis/root, same convention as training
      expects after batch_pose17_to_pose23 (Cartesian + RPY + gripper means + waist3 in last 5 of 17).
    - Coordinate frame: translation & orientation of EE in *pelvis/root* frame (not world), to reduce floating-base drift.

    If link names differ on a custom USD, extend the candidate tuples below.
    """
    try:
        data = env.scene["robot"].data
        ...
        root_i = _g1_root_body_index(body_names)
        l_i = _g1_first_body_index(
            body_names,
            ("left_wrist_yaw_link", "left_wrist_yaw", "L_ee", "left_arm_link7"),
        )
        r_i = _g1_first_body_index(
            body_names,
            ("right_wrist_yaw_link", "right_wrist_yaw", "R_ee", "right_arm_link7"),
        )

        b0 = 0
        quat_root = body_pose[b0, root_i, 3:7].unsqueeze(0)
        pos_root = body_pose[b0, root_i, :3]
        quat_l = body_pose[b0, l_i, 3:7].unsqueeze(0)
        pos_l = body_pose[b0, l_i, :3]
        quat_r = body_pose[b0, r_i, 3:7].unsqueeze(0)
        pos_r = body_pose[b0, r_i, :3]

        quat_root_w = ensure_quat_w_first(quat_root, assume_w_first=True)
        quat_l_w = ensure_quat_w_first(quat_l, assume_w_first=True)
        quat_r_w = ensure_quat_w_first(quat_r, assume_w_first=True)

        R_root = quat_to_rot_matrix(quat_root_w)[0]
        R_l = quat_to_rot_matrix(quat_l_w)[0]
        R_r = quat_to_rot_matrix(quat_r_w)[0]

        p_l_root = R_root.T @ (pos_l - pos_root)
        p_r_root = R_root.T @ (pos_r - pos_root)
        R_l_rel = R_root.T @ R_l
        R_r_rel = R_root.T @ R_r

        rpy_l = rot_mat_to_rpy_for_pose17(R_l_rel.unsqueeze(0))[0]
        rpy_r = rot_mat_to_rpy_for_pose17(R_r_rel.unsqueeze(0))[0]

        row = joint_pos[b0]
        waist_list = []
        for wn in ("waist_yaw_joint", "waist_roll_joint", "waist_pitch_joint"):
            waist_list.append(row[joint_names.index(wn)])
        wstack = torch.stack(waist_list)
        gL = _g1_gripper_scalar_from_joints(joint_names, row, left=True)
        gR = _g1_gripper_scalar_from_joints(joint_names, row, left=False)
        ...
        if p17.numel() != 17:
            return None, None
        ee6 = pose17_to_pose23_torch(p17)
        return p17.detach().cpu().numpy().tolist(), ee6.detach().cpu().numpy().tolist()
    except Exception as e:
        print(f"[g1_state] compute_g1_ee_qpos_and_state_6d failed: {e}")
        return None, None
